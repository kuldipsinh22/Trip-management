import {db} from "../db.js";

export const getInquiries=(req,res)=>{
    const query="SELECT * FROM `inquiries`";
    db.query(query,(err,data)=>{
        if(err){
            return res.json(err)
        }
        else{
            return res.json(data)
        }
    })
}

export const getInquirie=(req,res)=>{
    const query="SELECT * FROM `inquiries` WHERE inquiries_id=?";
    db.query(query,[req.params.id],(err,data)=>{
        if(err) return res.json(err)
        return res.json(data[0])
    })
}

export const insertInquirie=(req,res)=>{
    const query="INSERT INTO `inquiries`(`question`, `status`, `entry_date`, `update_date`, `entry_by`) VALUES(?)";
    const values=[
        req.body.question,
        req.body.status,
        req.body.entry_date,
        req.body.update_date,
        req.body.entry_by
    ];
    console.log(query);
  console.log(values);
  db.query(query,[values],(err,data)=>{
    if(err) return res.json(err)
    return res.json("New record has been inserted")
    })
}

export const updateInquirie=(req,res)=>{
    const query="UPDATE `inquiries` SET `question`=?,`status`=?,`entry_date`=?,`update_date`=?,`entry_by`=? WHERE inquiries_id=?"
    const values=[
        req.body.question,
        req.body.status,
        req.body.entry_date,
        req.body.update_date,
        req.body.entry_by
    ];
    db.query(query, [...values, req.params.id], (err, data) => {
        if(err) return res.json(err)
        return res.json("New record has been updated")
    })
}

export const deleteInquirie=(req,res)=>{
    const query="DELETE FROM `inquiries` WHERE inquiries_id=?";
    db.query(query,[req.params.id],(err,data)=>{
        if(err) return res.json(err);
     return res.json("Record has been deleted")
    })
}