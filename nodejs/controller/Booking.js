...
export const insertBooking=(req,res)=>{
    const query="INSERT INTO `manage_bookings`(`user_id`, `package_id`, `company_id`, `total_person`, `price`, `adv_payment`, `status`, `entary_date`, `update_date`, `entry_by`) VALUES(?)";
    const values=[
        req.body.user_id,
        req.body.company_id,
        req.body.package_id,
...
export const updateBooking=(req,res)=>{
    const query="UPDATE `manage_bookings` SET `user_id`=?,`package_id`=?,`company_id`=?,`total_person`=?,`price`=?,`adv_payment`=?,`status`=?,`entary_date`=?,`update_date`=?,`entry_by`=? WHERE booking_id=?"
    const values=[
        req.body.user_id,
        req.body.package_id,
        req.body.company_id,
